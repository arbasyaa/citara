...
@section('title', __('site.transport'))
...
@section('content')
    ...
    <section class="container mx-auto px-6 -mt-8 relative z-20">
        <div class="search-bar rounded-2xl p-6 max-w-4xl mx-auto">
            <form action="{{ route('transportasi.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('site.search') }} {{ strtolower(__('site.transport')) }}..." class="w-full px-6 py-4 rounded-xl text-gray-800 focus:outline-none bg-gray-50 border-2 border-transparent focus:border-blue-500 text-lg transition-all">
                </div>
    ...
    <section class="container mx-auto px-6 py-16 section-reveal">
        @if($items->isEmpty())
            <div class="text-center py-20">
                ...
                <h3 class="text-2xl font-bold text-gray-900 mb-2">{{ __('site.no_transport') }}</h3>
                    ...
                    <article class="dest-card bg-white rounded-2xl shadow-lg overflow-hidden group">
                        <div class="relative h-64 overflow-hidden">
                            @if(!empty($t->thumbnail))
                                <img src="{{ \App\Services\ImageUrl::url($t->thumbnail) }}" alt="{{ $t->nama }}" class="dest-image w-full h-full object-cover">
                            @else
                                <div class="w-full h-full bg-gradient-to-br from-blue-400 to-purple-500"></div>
                            @endif
                            <div class="absolute top-4 right-4">
                                <span class="badge px-4 py-2 rounded-full text-sm font-semibold text-gray-800 shadow-lg">{{ $t->tipe ?: __('site.transport') }}</span>
                            </div>
                        ...
                        <div class="p-6">
                            <h3 class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors">{{ $t->nama }}</h3>
                            <p class="text-gray-600 mb-4 line-clamp-3">{{ Str::limit($t->deskripsi, 150) }}</p>
                            @if($t->rute)
                                <div class="flex items-start text-gray-500 mb-4">
                                    ...
                                    <span class="text-sm">{{ Str::limit($t->rute, 40) }}</span>
                                </div>
                            @endif
                            <a href="{{ route('transportasi.show', $t->slug) }}" class="btn-view inline-flex items-center gap-2 px-6 py-3 text-white rounded-xl font-semibold shadow-lg">
                                {{ __('site.read_more') }}
