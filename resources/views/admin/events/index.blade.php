...
@section('content')
        <div class="flex flex-wrap gap-3 justify-between items-center mb-4">
            <h1 class="text-xl font-semibold">Kegiatan Kalender</h1>
            <a href="{{ route('panel.events.create') }}" class="px-4 py-2 rounded-md bg-indigo-600 text-white hover:bg-indigo-700">Kegiatan Baru</a>
        </div>
        ...
        <table class="min-w-full bg-white shadow rounded overflow-hidden">
            <thead>
                <tr>
                    <th class="px-4 py-3 text-left bg-gray-50">Bulan</th>
                    <th class="px-4 py-3 text-left bg-gray-50">Judul</th>
                    <th class="px-4 py-3 text-left bg-gray-50">Rentang Tanggal</th>
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2 text-center">
                        ...
                        <form method="POST" action="{{ route('panel.events.destroy', $event) }}" class="inline" onsubmit="return confirm('Hapus?')">
                            @csrf
                            @method('DELETE')
